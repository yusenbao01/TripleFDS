...
class Colorize(object):
    ...
    def border(self, alpha, size, kernel_type="RECT"):
        ...
        kdict = {"RECT": cv2.MORPH_RECT, "ELLIPSE": cv2.MORPH_ELLIPSE, "CROSS": cv2.MORPH_CROSS}
    ...
    def resize_im(self, im, osize):
        return np.array(Image.fromarray(im).resize(osize[::-1], Image.BILINEAR))
    ...
    def color(self, text_arr, bg_arr, fg_col, bg_col, colorsRGB, colorsLAB, min_h, param):
        ...
        mask = np.zeros_like(l_text.alpha)
        for i in range(len(layers)):
            mask = cv2.bitwise_or(mask, layers[i].alpha)
        ...
        elif param["difficulty"] == 0:
            l_out = poisson_reconstruct.alpha_blend_images(l_normal.color.copy(), l_bg.color.copy(), mask)
